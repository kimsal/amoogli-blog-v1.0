		   			...
		   			if file:
		   				...
		   				for f in uploaded_files:
		   					...
		   					if imagename!="":
			   					...
			   					help=help+1
		   				obj=Post(request.form['title'],request.form['description'],request.form['category_id'],filename,request.cookies.get('blog_id'),0,images,request.form["short_description"])
			        	status=Post.add(obj)
				        if not status:
...
def index(pagination=1):
	...
	return render_template(template+'/index.html',slide=slide,members=members,events=events,form=form,page_name='home',posts_top=posts_top,home_posts=home_posts,posts_bottom = posts_bottom,pagin=int(pagin),current_pagin=int(pagination))
...
def single(slug='',pagination=1):
	...
	return render_template(template+'/single.html',events=events,page_name='single',related_posts=related_posts,post_object=post_object)
